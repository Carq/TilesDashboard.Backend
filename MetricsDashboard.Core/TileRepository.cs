    ...
    public class TileRepository : ITileRepository
    {
        ...
        public async Task<IList<ITile>> GetMetricTilesAsync(CancellationToken cancellationToken)
        {
            var query = from tile in _tiles.AsQueryable().Where(t => t.TileType == TileType.Metric)
                ...
                join metric in GetMetrics<decimal>() on tile.Id equals metric.TileId into metrics
                select new { tile.Name, setting,  metrics };
        ...
        public async Task<IList<ITile>> GetBooleanTilesAsync(CancellationToken cancellationToken)
        {
            var query = from tile in _tiles.AsQueryable().Where(t => t.TileType == TileType.Boolean)
                join metric in GetMetrics<bool>() on tile.Id equals metric.TileId into metrics
        ...
        private IMongoQueryable<MetricEntity<TValue>> GetMetrics<TValue>() =>
            _database.GetCollection<MetricEntity<TValue>>(CollectionNames.Metrics).AsQueryable();
    }
}
